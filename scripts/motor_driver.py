...
class Motor():
    ...
    def set_speed(self, speed: float):
        current_dir = self.pi.read(self.dir_pin)
        current_speed = self.pi.get_PWM_dutycycle(self.pwm_pin)
        ...
        if dir_value != current_dir and max(current_speed, speed) > 0.05*self.MAX_SPEED:
            self.pi.write(self.pwm_pin, 0)
            rospy.sleep(0.1)  # 100ms

        # Set direction
        self.pi.write(self.dir_pin, dir_value)
        # Set duty cycle to speed
        self.pi.set_PWM_dutycycle(self.pwm_pin, speed*100)
